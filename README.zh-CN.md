...
切换模式复用分析缓存——先导深度，再导骨架，每次追加导出只需片段的实时时长。 ZIP 内为 UTF-8 文件名（Finder 双击、Windows 资源管理器解压均正常）；命令行解压建议用 `ditto -x -k 文件.zip 目录/` 或 7-Zip——macOS 自带的老版 unzip 会把中文文件名显示成乱码（数据无损）。
